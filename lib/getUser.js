import { cookies } from 'next/headers'
import jwt from 'jsonwebtoken'

export async function getUserFromToken() {
  try {
    const cookieStore = await cookies()
    const token = cookieStore.get('journals_token')
    if (!token) return null
    const decoded = jwt.verify(token.value, process.env.JWT_SECRET || 'travitrade_secret_2025')
    console.log('DECODED TOKEN:', JSON.stringify(decoded))

    const isAdmin = decoded.email === 'altuveronalbis@gmail.com'
    const rawPlan = decoded.plan || 'free'
    const isPro = ['pro', 'free_full', 'pro_monthly', 'pro_annual', 'admin'].includes(rawPlan)
    const plan = isAdmin ? 'pro' : (isPro ? 'pro' : (rawPlan || 'free'))
    const userId = decoded.userId || decoded.id || decoded.sub

    return {
      userId,
      id: userId,
      email: decoded.email,
      nombre: decoded.nombre,
      plan,
      hasFullAccess: isAdmin || isPro
    }
  } catch(e) {
    console.error('Token error:', e.message)
    return null
  }
}
